...
- **Possible rename: "Reeler Dealer."** BuzzFeed already has a game called Reel Deal, so we may need a new name. "Reeler Dealer" (a play on "wheeler dealer") doesn't look taken. Nothing is renamed yet. Before switching, run a trademark search and check domain and app-store availability, then update the page `<title>`, heading, share-text, README, and repo name.
